...
def build_llms_txt(spec):
    ...
    lines = [
        ...
        "",
        "`variations` is capped at 10: enough to seed traversal, and small "
        "enough not to matter on a single-product question (100 of them were "
        "half of a compact record). When the family is larger the object "
        "carries `variations_truncated: true` and a note giving both counts "
        "and how to build the rest: an array quietly shorter than its own "
        "`variations_total` is exactly what `_truncated` exists to prevent "
        "on the list endpoints. The record's `variation_summary` names every "
        "attribute value, so you rarely need the rest. `_links` is omitted "
        "when you pass `fields=`, since you asked for named keys.",
        "",
    ...
    lines += [
        ...
        "",
        "### The deals feed filters by id, and tells you the ids",
        "",
        "`/v2/deals` takes `categories` (a department name as Amazon shows it, "
        "such as `Electronics`, or its id), `brands` (brand ids only), "
        "`min_product_star_rating=4`, `min_price` / `max_price` / "
        "`min_discount`, `prime_exclusive` and `prime_early_access`. Every "
        "deals answer carries `available_filters` -- the category and brand "
        "ids that marketplace accepts, with names -- plus `filters_applied` "
        "and `filters_ignored` saying what actually took effect, and "
        "`next_offset` for the next page of 30 (`null` when the feed ends). "
        "An unknown category name is a free 400 that lists the valid ones. "
        "For a brand by name use `/search?brand=<name>&today_deals=true`.",
        "",
        "    https://agent.apiguru.app/agent/v1/v2/deals?geo=US&categories=Electronics&min_discount=40",
        "",
        ...
        "",
        "**Variants are summarised, not listed.** A listing family can have "
        "hundreds of siblings -- the Crocs Classic Clog has 733 -- and the "
        "ASIN-keyed maps that hold them (`variation` on details, "
        "`product_variations` on the batch endpoint) were 52 KB and 130 KB "
        "on it, most of the record and none of it about the product asked "
        "for. Both records now carry `variation_summary` instead: the "
        "attributes that vary, the distinct values of each, the sibling "
        "count, this ASIN's own values and `parent_asin`, about 1.5 KB at "
        "worst. The maps are still in the full record and in "
        "`_omitted_fields`; ask for one by name (`fields=variation`) when you "
        "need to resolve a value to a sibling ASIN.",
        "",
        ...
        "bulk arrays (`variation`, `all_product_variations`, "
        "`product_reviews`, image lists, `tech_specs`) come last. Truncating "
        "the response is therefore safe -- you lose the bulk, not the "
        "summary. It used to be alphabetical, which put 53 KB of variations "
        "ahead of the 1 KB review summary.",
        "",
...
PROBE_RULES_BEGIN = "# --- BEGIN GENERATED RULES"
PROBE_RULES_END = "# --- END GENERATED RULES ---"

# The keys of a JSON-Schema property that probe.py's local check acts on.
# Everything else (descriptions, defaults) stays out so the block is small.
_RULE_KEYS = ("type", "enum", "pattern", "minimum", "maximum")


def build_probe_rules(spec):
    """Python source for probe.py's GEOS/RULES block, from the spec.

    One rule per endpoint: its required parameters and, per parameter, the
    type, enum, pattern and range the API publishes -- the same declarations
    gateway/validate.py checks, plus the mcp_params (limit/compact/fields)
    the gateway honours over REST. A batch parameter carries `item_pattern`
    and `max_items` so the script can name the offending entry and refuse an
    over-long list before it costs anything. Emitted with repr(), never
    json.dumps(): true/false/null are not Python.
    """
    geos = list(spec["geos"])
    lines = [
    ...
    for ep in spec["endpoints"]:
        schema = ep.get("input_schema") or {}
        props = dict(schema.get("properties") or {})
        props.update(ep.get("mcp_params") or {})
        rendered = []
        for name, prop in props.items():
            rule = {k: prop[k] for k in _RULE_KEYS if k in prop}
            if ep.get("count_param") == name:
                rule.pop("pattern", None)
                rule["item_pattern"] = ep["item_pattern"]
                rule["max_items"] = ep["max_items"]
            if not rule or rule == {"type": "string"}:
                continue
            parts = []
            for key, value in rule.items():
                literal = "GEOS" if key == "enum" and list(value) == geos else repr(value)
                parts.append(f"{key!r}: {literal}")
            rendered.append(f"{name!r}: {{{', '.join(parts)}}}")
        lines.append(f"    {ep['path']!r}: {{")
        lines.append(f"        'required': {list(schema.get('required') or [])!r},")
        lines.append("        'params': {")
...
def sync_probe_rules(spec, probe_path):
    """Rewrite the generated block in probe.py between its two markers."""
    if not probe_path.exists():
        return
    text = probe_path.read_text(encoding="utf-8")
    start = text.find(PROBE_RULES_BEGIN)
    end = text.find(PROBE_RULES_END)
    if start < 0 or end < 0 or end < start:
        raise SystemExit(f"{probe_path}: generated-rules markers missing")
    head_end = text.index("\n", start) + 1
    new = text[:head_end] + build_probe_rules(spec) + "\n" + text[end:]
    if new != text:
        probe_path.write_text(new, encoding="utf-8")
        print(f"rules -> {probe_path}")
    # The block must still be importable Python.
    compile(new, str(probe_path), "exec")
...
def sync_versions(spec):
    ...
    # probe.py stamps the skill version into its User-Agent and into every
    # feedback entry, so a wall report can be read against the text that
    # produced it.
    rewrite(
        "skill/apiguru-amazon-data/scripts/probe.py",
        r'^SKILL_VERSION = "[^"]+"',
        f'SKILL_VERSION = "{version}"',
        1,
    )

    sync_probe_rules(spec, root / "skill" / "apiguru-amazon-data" / "scripts" / "probe.py")
