...
from .traffic import McpTrafficMiddleware
...
# Headers a browser-based MCP client sends, and the one it must be allowed to
# read back to keep a session.
CORS_ALLOW_HEADERS = (
    "Content-Type", "Authorization", "X-API-KEY",
    "Mcp-Session-Id", "MCP-Protocol-Version", "Last-Event-ID",
    # 2026-07-28 clients mirror the method and tool name into headers.
    "Mcp-Method", "Mcp-Name",
)
CORS_EXPOSE_HEADERS = ("Mcp-Session-Id", "MCP-Protocol-Version")


def accept_shortfall(method: str, accept: str | None) -> tuple[str, ...] | None:
    """The media types Streamable HTTP needs and this Accept header lacks.

    None means "not obviously broken" -- no header at all, a wildcard, or
    everything required already named. The SDK's own check still runs after
    this and still decides; being conservative here means this can only ever
    improve the message, never reject something that would have been served.
    """
    ...
    if not header:
        return None
    if "*/*" in header or "application/*" in header or "text/*" in header:
        return None
    needed = ("application/json", "text/event-stream") if method == "POST" \
        else ("text/event-stream",)
    missing = tuple(t for t in needed if t not in header)
    return needed if missing else None


class AcceptGuardMiddleware(BaseHTTPMiddleware):
    """Answer the Accept-header 406 with something an agent can act on.

    Streamable HTTP requires `Accept: application/json, text/event-stream` on
    POST and `text/event-stream` on GET. A client that sends neither got a
    bare 406 with no body worth reading, and simply stopped -- one did, on
    2026-09-06, and never reached a tool. The SDK's own check still runs and
    still decides; this only fires when the header is unambiguously the
    problem, so it can add a `next_step` without ever rejecting a request the
    SDK would have served.
    """
    ...
    async def dispatch(self, request: Request, call_next):
        if request.url.path in self._paths:
            needed = accept_shortfall(request.method, request.headers.get("Accept"))
            if needed is not None:
                return JSONResponse(
                    {
                        "error": (
                            "This endpoint speaks MCP over Streamable HTTP, which "
                            f"requires an Accept header naming {' and '.join(needed)}."
                        ),
                        "code": "unacceptable_accept_header",
                        "http_status": 406,
                        "billed": False,
                        "retryable": True,
                        "received_accept": request.headers.get("Accept"),
                        "next_step": (
                            "Retry the same request with "
                            f"`Accept: {', '.join(needed)}`. If you are not writing an "
                            "MCP client, use the plain REST API at "
                            "https://agent.apiguru.app/agent/v1 instead, which needs "
                            "no special headers."
                        ),
                        "docs": api_info()["docs_url"],
                    },
                    status_code=406,
                )
        return await call_next(request)
...
def _add_cors(app, origins: list[str]) -> None:
    """Make a cross-origin preflight answer consistently.

    `OPTIONS /mcp` used to answer 405 over HTTP/1.1 and 403 over HTTP/2 --
    two different refusals for the same request, neither of them a preflight
    response, so a browser-based MCP client could not connect at all and had
    no way to find out why. The origin list is the same one the transport's
    DNS-rebinding protection already enforces, so this makes the existing
    policy answerable rather than widening it; APIGURU_MCP_ALLOWED_ORIGINS
    still decides what that policy is.
    """
...
def create_app(streamable_http_path: str = "/mcp", host: str = "127.0.0.1"):
    ...
    mcp_paths = (streamable_http_path, ACCOUNT_PATH)
    cors_origins = sorted(security.allowed_origins or [])

    if not oauth.enabled():
        # Added last-first: Starlette runs add_middleware in reverse, so CORS
        # must go on after the guard to sit outermost and answer preflight.
        open_app.add_middleware(RequestContextMiddleware)
        open_app.add_middleware(AcceptGuardMiddleware, paths=mcp_paths)
        _add_cors(open_app, cors_origins)
        # Outermost: sees every exchange, preflights and 406s included.
        open_app.add_middleware(McpTrafficMiddleware, paths=mcp_paths)
        return open_app
    ...
    # Outermost first. CORS answers the preflight before anything can refuse
    # it, and the Accept guard replaces a bare 406 with a document that says
    # which header to send -- both before authentication, which a preflight
    # carries no credentials for anyway.
    app = Starlette(
        routes=[
        ...
        middleware=[
            Middleware(McpTrafficMiddleware, paths=mcp_paths),
            Middleware(
                CORSMiddleware,
            ...
            Middleware(AcceptGuardMiddleware, paths=mcp_paths),
